...
with abas[1]:
    ...
    if jogos_salvos_atual:
        ...
        if pool_salvo:
            ...
            # --- PERFORMANCE DAS FIXAS (COLE ABAIXO DO MARKDOWN DO POOL) ---
            fixas_salvas = jogos_salvos_atual[0].get('fixas_utilizadas', [])
            if fixas_salvas and resultado_alvo:
                st.markdown("### 📌 PERFORMANCE DAS FIXAS")
                ac_f = 0
                h_f = '<div style="background:#1a1a1a; padding:15px; border-radius:10px; border:2px solid #d4af37; margin-bottom:15px;">'
                for df in sorted(fixas_salvas):
                    cor_f = "#28a745" if df in resultado_alvo else "#ff4b4b"
                    if df in resultado_alvo: ac_f += 1
                    h_f += f'<span style="background:{cor_f}; color:white; padding:5px 10px; border-radius:50%; margin-right:5px; border:1px solid white;">{df:02d}</span>'
                h_f += f'<br><br><b style="color:#d4af37;">🎯 ACERTOS NAS FIXAS: {ac_f}</b></div>'
