...
if not st.session_state.auth:
    ...
    st.stop()
# =====================================================================
# 🚀 MOTOR DE IGNIÇÃO: BOOT AUTÔNOMO E RADAR DE ARQUIVOS LOCAIS
# =====================================================================
if 'boot_concluido' not in st.session_state:
    st.session_state.boot_concluido = False

if not st.session_state.boot_concluido:
    import os
    import glob
    import time
        
    # 1. O Radar de Arquivos (Resolve o problema do (1), (2), (3)...)
    pasta_cofre = r"C:\Users\Eder\Desktop\LOTO"
    padrao_busca = os.path.join(pasta_cofre, "Cofre*.json")
    arquivos_encontrados = glob.glob(padrao_busca)
        
    placeholder_boot = st.empty() # Cria um contêiner temporário que vai desaparecer depois
        
    with placeholder_boot.container():
        if arquivos_encontrados:
            # Pega estritamente o arquivo mais novo baseado no relógio do Windows
            arquivo_mais_recente = max(arquivos_encontrados, key=os.path.getmtime)
            nome_arquivo = os.path.basename(arquivo_mais_recente)
                
            st.info(f"📂 Boot Local: Carregando banco de dados mais recente encontrado ({nome_arquivo})...")
            try:
            ...
            st.warning(f"⚠️ Nenhum banco de dados 'Cofre*.json' encontrado em {pasta_cofre}. Iniciando sistema limpo.")
            arquivo_mais_recente = None # Marca como nulo para não tentar salvar depois

        # 2. O Sensor de Gap (Acesso invisível à Caixa)
        historico = st.session_state.data.get("historico_dados", [])
        if historico:
            ultimo_salvo = int(historico[-1]["concurso"])
            st.info(f"📡 Boot de Rede: Conectando aos servidores oficiais... (Seu último concurso é o {ultimo_salvo})")
            ...
            try:
                ...
                if ultimo_salvo < ultimo_oficial:
                    ...
                    st.warning(f"🛸 Sincronizando {qtd} concursos que ocorreram na sua ausência...")
                    barra_boot = st.progress(0)
                    texto_boot = st.empty()
                        
                    sucessos = 0
                    for i, num in enumerate(concursos_faltantes):
                        texto_boot.text(f"Baixando e auditando concurso {num} ({i+1}/{qtd})...")
                        try:
                            ...
                            if 'concurso' in res_conc:
                                ...
                                st.session_state.data["historico_dados"].append({
                                    "concurso": num, "dezenas": dezenas_sorteadas, "data": res_conc['data']
                                })
                                sucessos += 1
                                    
                                # Auditoria Inteligente (Paga prêmios e treina IA)
                            ...
                            pass # Ignora falhas isoladas de rede
                            
                        barra_boot.progress((i + 1) / qtd)
                        time.sleep(0.4)
                            
                    # 3. AUTO-SAVE (Salva a atualização direto no seu Windows sem precisar de botão!)
                    if sucessos > 0 and arquivo_mais_recente:
                        st.success("Sincronização concluída! Gravando dados atualizados no seu HD...")
                        try:
                            with open(arquivo_mais_recente, 'w', encoding='utf-8') as f:
                                json.dump(st.session_state.data, f, ensure_ascii=False, indent=4)
                        except Exception as e:
                            st.error(f"Erro ao salvar no HD: {e}")
                                
            except Exception as e:
                st.toast("Modo Offline ativado: Sem conexão com a Caixa para sincronização de boot.")
        
    # Encerra o processo de Boot e limpa a tela de carregamento
    st.session_state.boot_concluido = True
    placeholder_boot.empty()
    st.rerun()
# =====================================================================    
