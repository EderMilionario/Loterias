...
with abas[0]:
    ...
    # --- [INÍCIO DO INDICADOR DE CLIMA CORRIGIDO] ---
    if mod == "Lotofácil":
        df_hist = st.session_state.get('df_resultados')
        
        # Se houver histórico, analisa. Se não, avisa.
        if df_hist is not None and not df_hist.empty:
            ...
            if cenario_atual == "ESTAVEL":
                ...
                st.info("💡 **DICA PERITA:** Use **8 FIXAS** no slider abaixo.")
            elif cenario_atual == "ERRATICO":
                st.warning(f"⚠️ **CLIMA DETECTADO:** {cenario_atual}")
                st.info("💡 **DICA PERITA:** Use **7 FIXAS** no slider abaixo.")
            else:
                st.info(f"🌀 **CLIMA DETECTADO:** {cenario_atual}")
                st.info("💡 **DICA PERITA:** Recomendado entre **6 e 7 FIXAS**.")
        else:
            # Caso o backup ainda não tenha sido carregado
            st.info("⏳ Aguardando carregamento do Histórico para analisar o Clima...")
    # --- [FIM DO INDICADOR] ---
    c1, c2 = st.columns(2)
    with c1:
        if info_fech:
...
# --- [MOTOR DE DECISÃO UNIFICADO: ESTRATÉGIA + MATRIZ] ---
        ...
        if mod == "Lotofácil":
            ...
            elif "MATRIZ PERITA 20-16" in est_escolhida: tamanhos_detectados.append(20)    
